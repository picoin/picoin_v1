...
unsigned int nTargetSpacing = 67; // 67 seconds blocks
unsigned int nRetarget = 11;
unsigned int nStakeMinAge = 11 * 60 * 60; // 12 hours
...
static const int64_t nTargetTimespan = 3.14 * 60;
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // Picoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Picoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    int64_t nSubsidy = 3.14159265 * COIN;

    if (pindexBest->nHeight+1 == 1)
    {
     nSubsidy = 1254790 * COIN; //1,25 millions coins premine
    }

    else
    {
     nSubsidy = 3.14159265 * COIN; // Extra blocks to confirm and mature the premine
    }
...
// picoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes Picoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Picoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Picoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Picoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Picoin: track money supply and mint amount info
...
// Picoin: total coin age spent in transaction, in the unit of coin-days.
...
// Picoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // Picoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Picoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Picoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // Picoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // Picoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Picoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Picoin: getblocks may not obtain the ancestor block rejected
    ...
    // Picoin: if responsible for sync-checkpoint send it
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "14th September 1473808";
        CTransaction txNew;
        txNew.nTime = 1473808166;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1473808166;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3184042;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x3408cdaa1ae49efe5d11f12320998193d8563444ceb9f12327470a52e7a34e6b"));
        block.print();
        ...
        // Picoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // Picoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Picoin: tell downloading node about the latest block if it's
